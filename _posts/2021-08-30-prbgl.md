...
$$ \Gamma^{-1} = D_k (WW_i - \mathbb{1}) D_k^{\dagger}. $$

It also appears that $$\Gamma^{-1}$$ is its own pseudo-inverse.

The determinant terms in the complex normal log density (the terms that aren't dependent on the STFT) are zero however, and it seems to be the case that the pseudo-covariance is degenerate.

Todo:
 * verify implementations for other `n, n_fft, hop_length`s
 * try to find a non-degenerate example and corresponding parameters
