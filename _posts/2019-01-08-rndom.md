...
## Random Stuff
 ...
 21. It's probably impossible to represent _monotonic_ functions and _positive_ functions using a basis function representation _**on \[\mathbb R^n\]**_ because the space of monotonoic and/or positive functions isn't closed under linear transformations, so it cannot be a subspace endowed with a basis! (an awesome coworker shared this thought) However, there can exist bases (e.g. using the Aitchison geometry) on simplexes, etc.
 ...
 23. Gaussian process on spheres are easy to simulate (e.g. by using arc lengths instead of the usual distance metric in the kernel).
