...
plot_frame <- copy(sessions[session_id == sample(unique_sessions, 1)])
p <- ggplot(plot_frame)
p <- p + aes(x = session_position, y = us_popularity_estimate)
p <- p + geom_point(aes(color = not_skipped))
p
...
<center> <img src="/images/spotify/trnmt.png" width="75%"> </center>
